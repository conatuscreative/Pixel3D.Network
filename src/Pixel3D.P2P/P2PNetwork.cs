...
namespace Pixel3D.P2P
{
    public class P2PNetwork
    {
        ...
        List<RemotePeer> networkDataErrorKickList = new List<RemotePeer>();
        ...
        int unconnectedProtocolErrorCount = 0;
        ...
        public int PortNumber { get { return netPeer.Port; } }
        public bool UsingKnownPort { get { return PortIsKnown(netPeer.Port); } }
        public bool PortIsKnown(int port) { return appConfig.KnownPorts.Contains(port);}
        ...
        {
            NetPeerConfiguration config = new NetPeerConfiguration(appConfig.AppId);
        ...
        {
            string ipOrHost = ipOrHostOptionalPort;
            int port = appConfig.KnownPorts[0]; // Could eventually add support for trying multiple known ports.

            if(ipOrHostOptionalPort.Contains(':'))
            {
                string[] split = ipOrHostOptionalPort.Split(':');
                if(split.Length != 2)
        ...
        public void ConnectToGame(INetworkApplication networkApplication, string playerName, byte[] playerData, IPEndPoint p2pServerEndPoint,
        ...
        /// NOTE: Pass "null" to disconnect without error - otherwise the passed reason is considered an error (TODO: This is ugly and should get fixed!).
        /// </summary>
        ...
        /// <summary>Disconnect and raise a <see cref="NetworkDisconnectionException"/>. Must be inside something that will catch it!</summary>
        ...
        internal IPeerManager peerManager = null;
        ...
        public bool IsActive { get { return peerManager != null; } }
        public bool IsServer { get { return peerManager != null && peerManager is P2PServer; } }
        public bool IsApplicationConnected { get { return IsActive && LocalPeerInfo != null && LocalPeerInfo.IsApplicationConnected; } }
        ...
        public void SideChannelTryVerifyAndConnect(IPEndPoint potentialEndpoint)
        {
            ...
            if(p2pClient != null)
            {
                p2pClient.TryToVerifyAndConnect(potentialEndpoint);
            }
        }
        ...
        /// </remarks>
        List<NetConnection> broadcastList = new List<NetConnection>();

        List<RemotePeer> remotePeerList = new List<RemotePeer>();
        public ReadOnlyList<RemotePeer> RemotePeers { get { return new ReadOnlyList<RemotePeer>(remotePeerList); } }
