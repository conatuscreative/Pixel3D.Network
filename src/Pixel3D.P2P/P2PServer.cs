...
namespace Pixel3D.P2P
{
    internal class P2PServer : IPeerManager
    {
        ...
        Dictionary<ulong, int> authTokens;
        ...
        internal int SideChannelAdd(ulong sideChannelId)
        {
            ...
            {
                do { token = random.Next(); } while(token == 0);
        ...
        List<PendingDisconnect> pendingDisconnects = new List<PendingDisconnect>();

        /// <summary>How long a disconnected client has to respond (prove they are alive) before they get kicked. Actual value is arbitrary.</summary>
        const double pendingDisconnectTimeout = 8; // seconds (8 seconds == 2x max RTT, due to lidgren 4sec RTT bug)


        void AddPendingDisconnect(int failedConnectFrom, int failedConnectTo)
        {
        ...
        void RemovePendingDisconnectsFor(int connectionId)
        {
            for(int i = pendingDisconnects.Count - 1; i >= 0; i--)
            {
                if(pendingDisconnects[i].failedConnectFrom == connectionId || pendingDisconnects[i].failedConnectTo == connectionId)
                {
        ...
        SortedList<int, PendingConnection> pendingConnections = new SortedList<int, PendingConnection>();

        /// <summary>How long a connecting client has to form all P2P network connections.</summary>
        const double pendingConnectionTimeout = 60; // seconds (calculated in "P2P Network.pptx")



        void StartPendingConnection(RemotePeer remotePeer)
        {
            Debug.Assert(!pendingConnections.ContainsKey(remotePeer.PeerInfo.ConnectionId));

            pendingConnections.Add(remotePeer.PeerInfo.ConnectionId, new PendingConnection(NetTime.Now + pendingConnectionTimeout, remotePeer));
        }


        void RemovePendingConnectionsFor(int connectionId)
        {
        ...
        void MarkConnectionMade(int fromConnectionId, int toConnectionId)
        {
            ...
            {
                throw new ProtocolException("Unexpected connection report from #" + fromConnectionId + " to #" + toConnectionId);
            }
        ...
        bool IsReady(PendingConnection pendingConnection)
        {
            if(!pendingConnection.remoteInitialisedRTT)
                return false;

            int pendingConnectionId = pendingConnection.remotePeer.PeerInfo.ConnectionId;
            ...
            for(int i = 0; i < locallyConnected.Count; i++)
            {
                int otherConnectionId = (locallyConnected[i].Tag as RemotePeer).PeerInfo.ConnectionId;
        ...
        void ReadyCheck(PendingConnection pendingConnection)
        {
        ...
        void ReadyCheckAll()
        {
            for(int i = pendingConnections.Values.Count - 1; i >= 0 ; i--) // <- reverse iterate, as ReadyCheck might remove stuff
            {
        ...
        void UpdatePendingConnections(double currentTime)
        {
            while(pendingConnections.Count > 0 && pendingConnections.Values[0].timeOut < currentTime)
            {
                // Timed out:
                PendingConnection pendingConnection = pendingConnections.Values[0];

                Debug.Assert(!IsReady(pendingConnection)); // <- did we miss a ready check?

                owner.Log("Timed out waiting for P2P setup to finish: " + pendingConnection.remotePeer.PeerInfo.ToString());
        ...
        List<NetConnection> broadcastTemp = new List<NetConnection>();

        List<NetConnection> ListNotApplicationConnected(NetConnection except)
        {
            broadcastTemp.Clear();
            foreach(var connection in locallyConnected)
            {
        ...
        List<NetConnection> ListApplicationConnected(NetConnection except)
        {
            broadcastTemp.Clear();
            foreach(var connection in locallyConnected)
            {
        ...
        internal P2PServer(P2PNetwork owner, bool hostMigrationValidatedByServer, int maxConnectionId)
        {
            ...
            List<RemotePeer> leavingPeers = new List<RemotePeer>();
            foreach(RemotePeer remotePeer in owner.RemotePeers)
            {
                Debug.Assert(remotePeer.PeerInfo.IsApplicationConnected);
                ...
                Debug.Assert(!(remotePeer.PeerInfo.IsServer && remotePeer.IsConnected)); // old server should not still be connected
            ...
            Debug.Assert(owner.LocalPeerInfo.IsApplicationConnected == true);
        ...
        internal void CompleteHostMigration()
        {
            // Send host-migration packet:
            {
                ...
                for(int i = 0; i < owner.RemotePeers.Count; i++)
                {
                    owner.RemotePeers[i].PeerInfo.WriteTo(message);
                }
        ...
        ClientDisconnections clientDisconnections;

        double hostMigrationTime;
        const double hostMigrationGraceTime = 40; // seconds


        void ValidateHostMigration()
        {
        ...
        void CheckDisconnectionForHostMigrationInvalidation(string disconnectReason)
        {
            if(clientDisconnections != null)
            {
