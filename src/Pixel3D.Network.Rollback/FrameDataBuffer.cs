...
namespace Pixel3D.Network.Rollback
{
    ...
    internal class FrameDataBuffer<T> : SortedList<int, T>
    {
        ...
        /// <summary>Remove all entries before the given frame (exclusive). Can change the result given by <see cref="TryGetLastBeforeOrAtFrame"/>.</summary>
        ...
        public int OldestStoredFrame { get { return Count > 0 ? Keys[0] : 0; } }
        public int NewestStoredFrame { get { return Count > 0 ? Keys[Count - 1] : 0; } }
        ...
        {
            T v;
            int debugCheck = TryGetLastBeforeOrAtFrame(frame, out v);
        ...
        {
            for(int i = 0; i < Count; i++) // search forwards
            {
        ...
        {
            int lookingForFrame = startingFrame;
